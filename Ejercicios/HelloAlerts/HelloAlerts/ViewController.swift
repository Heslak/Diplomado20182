...
class ViewController: UIViewController {
    ...
    private let centerView: ProfileView = {
        let cV = ProfileView()
        cV.backgroundColor = UIColor.yankeesBlue
    ...
    private let simpleBtn: UIButton = {
        let btn = UIButton(type: .system)
        btn.setTitle("Simple Alert", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: .bold)
        btn.tintColor = .white
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.addTarget(self, action: #selector(tapOkAlert), for: UIControlEvents.touchUpInside)
        return btn
    }()
    ...
    func setupView(){
        ...
        NSLayoutConstraint.activate([
            centerView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 10),
            centerView.bottomAnchor.constraint(equalTo: simpleBtn.topAnchor, constant: -10),
            centerView.leadingAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.leadingAnchor, constant: 10),
            centerView.trailingAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.trailingAnchor, constant: -10)])
